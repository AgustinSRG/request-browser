 ...
 * Request headers
 */
export type RequestHeaders = { [headerName: string]: string };
 ...
 */
export type RequestAuthenticationHandler = (request: Request<any, any>) => RequestHeaders;

/**
 * Authentication status for requests
 */
const RequestAuthenticationHandlers: RequestAuthenticationHandler[] = [];

/**
 * Adds request authentication handler
 * @param handler The request authentication handler
 */
export function addRequestAuthenticationHandler(handler: RequestAuthenticationHandler) {
    RequestAuthenticationHandlers.push(handler);
}

/**
 * Removes request authentication handler
 * @param handler The request authentication handler
 */
export function removeRequestAuthenticationHandler(handler: RequestAuthenticationHandler) {
    for (let i = 0; i < RequestAuthenticationHandlers.length; i++) {
        if (RequestAuthenticationHandlers[i] === handler) {
            RequestAuthenticationHandlers.splice(i, 1);
            return;
        }
    }
}
...
/**
 * Removes all request authentication handlers
 */
export function removeAllRequestAuthenticationHandlers() {
    while (RequestAuthenticationHandlers.length > 0) {
        RequestAuthenticationHandlers.pop();
    }
}

/**
 * Adds authentication headers to the request headers
 * @param headers The request headers
 * @param request The request
 */
function addAuthenticationHeaders(headers: RequestHeaders, request: Request<any, any>) {
    for (const handler of RequestAuthenticationHandlers) {
        const authenticationHeaders = handler(request);

        for (const h of Object.keys(authenticationHeaders)) {
            headers[h] = authenticationHeaders[h];
        }
    }
}
...
export class Request<Return_Type = any, Error_Handler = never> {
    ...
    private sendAsFetch(callback?: () => void) {
        ...
        const headers: RequestHeaders = Object.create(null);
        ...
        addAuthenticationHeaders(headers, this);
